...
for symbol in symbols:
  ...
  ##%%
    ...
    df_5m = day_candles
    start = df_5m.iloc[0]
    idx_low = df_5m['l'].idxmin()
    low = df_5m['l'].min()
    idx_high = df_5m['h'].idxmax()
    high = df_5m['h'].max()
    end = df_5m.iloc[-1]

    is_up_move_day = idx_low < idx_high
    index_order = [start.name, idx_low, idx_high, end.name] if is_up_move_day else [start.name, idx_high, idx_low, end.name]

    time_diffs = np.diff(index_order)

    # Pullback algorithm
    # 1. Start with a candle range defined as [start end] whereas start['l'] is lowest value and end['h'] is highest value
    # 2. Look for the highest high of a candle C_hh before the end of the range
    # 3. If C_hh is right before the current candle set end = C_hh and GOTO 2
    # 4. If there is a Gap between C_hh the next highest candle and the current end:
    #   a. Calculate the value range C_hh['h'] - min([C_hh end]) as C_ll and save it together with the time range [C_hh C_ll]
    #   b. Continue to evaluate the two ranges [start C_hh] and [C_ll end] from Step 1
    ...
    try:
      ...
      slope_df = df_5m.copy()
      slope_addplots = [ama_vwap_slope_plot, ema_vwap_slope_plot]
