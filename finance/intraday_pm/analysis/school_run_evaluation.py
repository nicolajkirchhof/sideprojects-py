...
while first_day < last_day:
  ...
  ref_range_start = first_day + indicator_range_start
  ref_range_end = first_day + indicator_range_end
  # print(
  #   f"getting data for {ref_range_start.isoformat()} - {ref_range_end.isoformat()}"
  # )
  df_ref_range = index_client_df.query(get_candles_range(ref_range_start, ref_range_end, symbol, '30m'))

  # End of day value at 16:00
  before_closing = first_day + datetime.timedelta(hours=15)
  closing = first_day + datetime.timedelta(hours=18)
  df_closing = index_client_df.query(get_candles_range(before_closing, closing, symbol))
  if df_ref_range:
