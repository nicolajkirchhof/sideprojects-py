...
for symbol in symbols[1:]:
  ...
  dfs_ref_range = []
  dfs_closing = []
  first_day = dateutil.parser.parse('2023-01-02T00:00:00').replace(tzinfo=tz)
  last_day = dateutil.parser.parse('2025-03-06T00:00:00').replace(tzinfo=tz)
  day_start = first_day
  prior_close = None
  df_raw = None
  ...
  while day_start < last_day:
    ...
    # get the following data for daily assignment
      ...
      df_follow.to_parquet(f'{symbol_directory}/{symbol}_{timerange}_{day_start.strftime("%Y-%m-%d")}_follow.parquet', index=False)
