...
def interactive():
    ...
    try:
        data = utils.SwingTradingData('SPY', datasource='offline')
        full_df = data.df_day
        title = 'SPY'
    ...
    if _GLOBAL_MAIN_WIN is None:
        ...
        # Tab 3: Monthly Statistics (Violin Plots)
        ...
        monthly_scroll_content = QtWidgets.QWidget()
    ...
    def update_monthly_violins():
        ...
        # Aggregate data for the selected year range
        df_range = full_df[(full_df.index.year >= start_year) & (full_df.index.year <= end_year)]
    ...
    def load_data_from_ui():
        ...
        try:
            ...
            start_year_cb.addItems([str(y) for y in all_years])
            end_year_cb.addItems([str(y) for y in all_years])
            start_year_cb.setCurrentText(str(all_years[0]))
            end_year_cb.setCurrentText(str(all_years[-1]))
