...
class DashboardQt(QtWidgets.QMainWindow):
    def __init__(self, df: pd.DataFrame):
        ...
        self.event_move = self._add_range(
            grid, "Event Move (Signed)", df, col="event_move", decimals=3, step=0.25, default_quantiles=(0.01, 0.99)
        )

        # 4. Breakout Price
        # Match matplotlib: min + 0.99 quantile init, with full data-derived hard range.
        self.event_price = self._add_range(
            grid, "Breakout Price", df, col="event_price", decimals=2, step=0.5, default_quantiles=(0.0, 0.99)
        )

        # 6-8. Momentum Filters
        self.mom_1m = self._add_range(grid, "1M_chg", df, col="1M_chg", decimals=1, step=1.0, default_quantiles=(0.01, 0.99))
        self.mom_3m = self._add_range(grid, "3M_chg", df, col="3M_chg", decimals=1, step=1.0, default_quantiles=(0.01, 0.99))
        self.mom_6m = self._add_range(grid, "6M_chg", df, col="6M_chg", decimals=1, step=1.0, default_quantiles=(0.01, 0.99))
        ...
        for ema_name in ["ema10", "ema20", "ema50", "ema100", "ema200"]:
            ...
            self.ema_filters[col] = self._add_range(
                grid, f"{ema_name} Dist", df, col=col, decimals=2, step=0.25, default_quantiles=(0.01, 0.99)
            )
        ...
        for ema_name in ["ema10", "ema20", "ema50", "ema100", "ema200"]:
            ...
            self.spy_ema_filters[col] = self._add_range(
                grid, f"SPY {ema_name} Dist", df, col=col, decimals=2, step=0.25, default_quantiles=(0.01, 0.99)
            )
        ...
        self.cond_range = self._add_range(
            grid=None, title="Cond. Range", df=df, col="__cond_placeholder__", decimals=1, step=0.5, default_quantiles=(0.01, 0.99),
            add_to_grid=False
        )
    ...
    def _add_year_range(self, grid: QtWidgets.QGridLayout, title: str, df: pd.DataFrame) -> MinMaxSpin:
        ...
        y0 = int(years.min()) if not years.empty else 2000
        ...
        # Match matplotlib defaults: start at 2010, end at max year
        min_box.setValue(2010)
        max_box.setValue(y1)
    ...
    def _add_range(
        ...
        decimals: int,
        step: float,
        default_quantiles: tuple[float, float] = (0.0, 1.0),
        *,
        ...
        if not series.empty:
            ...
            q0, q1 = default_quantiles
            v0 = float(series.quantile(q0))
