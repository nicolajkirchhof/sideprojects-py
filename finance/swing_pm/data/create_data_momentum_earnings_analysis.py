...
#   Daily columns are from -60 to 60 whereas 0 is the event day (Total: 121 days)
#   Weekly columns are from -12 to 12 whereas 0 is the event week (Total: 25 weeks)
# Tracked names
...
# ---------------------------------------------------------------------------
# Helper: default meta dict for a new event
# ---------------------------------------------------------------------------
def _new_event_meta(reaction_idx: int) -> dict:
    return {
...
# ---------------------------------------------------------------------------
# Helper: add event to events_map with boundary + volume check
# ---------------------------------------------------------------------------
def _try_add_event(
    events_map: dict,
    df_day: pd.DataFrame,
    reaction_idx: int,
    event_type: str,
    disregarded_count: int,
    extra_meta: dict | None = None,
) -> int:
    """Add event to map if it passes boundary and volume checks. Returns updated disregarded_count.

    Note: boundary failures return disregarded_count unchanged — only volume
    failures increment the counter, matching the original per-event logic.
    """
    ...
    meta = events_map.get(reaction_date, _new_event_meta(reaction_idx))
    meta['reaction_idx'] = reaction_idx
    meta['event_types'].add(event_type)
    if extra_meta:
...
for i, ticker in enumerate(symbols_to_process):
    ...
    for i_earn, earnings_event in df_earnings.iterrows():
        ...
        day_high = row_day.h if 'h' in df_day.columns else np.nan
        day_low = row_day.l if 'l' in df_day.columns else np.nan
        day_close = row_day.c
        if pd.notna(day_high) and pd.notna(day_low) and pd.notna(day_close) and (day_high - day_low) > 0:
            meta['close_in_range'] = (day_close - day_low) / (day_high - day_low)
        else:
            meta['close_in_range'] = np.nan

        # Consecutive beats (from pre-computed map)
        meta['consecutive_beats'] = earnings_consecutive_beats.get(earnings_event.date, 0)
    ...
    if 'atrp20' in df_day.columns and 'pct' in df_day.columns:
        ...
        for reaction_date in df_day.index[atrp_condition]:
            ...
            disregarded_count = _try_add_event(
                events_map, df_day, reaction_idx, 'atrp_breakout', disregarded_count)
    ...
    if ('h' in df_day.columns) and ('c' in df_day.columns) and (not df_day.empty):
        ...
        for idx in range(1, len(df_day)):
            ...
            if consolidation_days >= 5:
                disregarded_count = _try_add_event(
                    events_map, df_day, idx, 'green_line_breakout', disregarded_count,
                    extra_meta={'consolidation_days': float(consolidation_days)})
    ...
    if (
        ...
        for reaction_date in df_day.index[bb_condition]:
            reaction_idx = df_day.index.get_loc(reaction_date)
            disregarded_count = _try_add_event(
                events_map, df_day, reaction_idx, 'bb_lower_touch', disregarded_count)

    # --- 5. Identify Episodic Pivots (Type A — gap ≥ 10%, RVOL ≥ 5×, close in top/bottom 25%) ---
    if 'gappct' in df_day.columns and 'rvol20' in df_day.columns and 'h' in df_day.columns and 'l' in df_day.columns:
        for idx in range(OFFSET_DAYS, len(df_day) - OFFSET_DAYS):
            row = df_day.iloc[idx]
            gap = row.gappct if pd.notna(row.gappct) else 0
            rvol = row.rvol20 if pd.notna(row.rvol20) else 0
            h_val = row.h
            l_val = row.l
            c_val = row.c
            day_range = h_val - l_val if (pd.notna(h_val) and pd.notna(l_val)) else 0

            if abs(gap) < 10 or rvol < 5 or day_range <= 0:
                continue

            close_pos = (c_val - l_val) / day_range if pd.notna(c_val) else 0.5
            # Long EP: gap up + close in top 25%; Short EP: gap down + close in bottom 25%
            is_long_ep = gap >= 10 and close_pos >= 0.75
            is_short_ep = gap <= -10 and close_pos <= 0.25

            if is_long_ep or is_short_ep:
                disregarded_count = _try_add_event(
                    events_map, df_day, idx, 'episodic_pivot', disregarded_count)

    # --- 6. Identify Pre-Earnings Events (T-14 before earnings) ---
    # For PM-03: create an event 14 trading days before each earnings date
    if not df_earnings.empty:
        for _, earnings_event in df_earnings.iterrows():
            idx_arr = df_day.index.get_indexer([earnings_event.date], method='nearest')
            if len(idx_arr) == 0:
                continue
            earn_idx = idx_arr[0]
            # T-14 trading days before earnings
            pre_idx = earn_idx - 14
            if pre_idx < 0:
                continue

            # Only add if stock is in Stage 2 (above 50d and 200d SMA) and has beat history
            pre_row = df_day.iloc[pre_idx]
            ma50_ok = pre_row.get('ma50_dist', np.nan)
            ma200_ok = pre_row.get('ma200_dist', np.nan)
            if not (pd.notna(ma50_ok) and ma50_ok > 0 and pd.notna(ma200_ok) and ma200_ok > 0):
                continue

            consecutive = earnings_consecutive_beats.get(earnings_event.date, 0)
            # PM-03 requires beat ≥ 3 of last 4 quarters — proxy: consecutive_beats ≥ 3
            if consecutive < 3:
                continue

            disregarded_count = _try_add_event(
                events_map, df_day, pre_idx, 'pre_earnings', disregarded_count,
                extra_meta={'pre_earnings_target_date': earnings_event.date})

    # --- 7. Identify EMA Reclaim Events (Type C — pullback to MA then recovery) ---
    # Condition: ma10_dist or ma20_dist crosses zero from above while ma50_dist > 0, rvol20 < 1.0
    if all(c in df_day.columns for c in ['ma10_dist', 'ma20_dist', 'ma50_dist', 'rvol20']):
        for idx in range(OFFSET_DAYS + 1, len(df_day) - OFFSET_DAYS):
            curr = df_day.iloc[idx]
            prev = df_day.iloc[idx - 1]

            # MA stack must be intact: ma50_dist > 0 (above 50d SMA)
            if not (pd.notna(curr.ma50_dist) and curr.ma50_dist > 0):
                continue

            # Check for reclaim: prev day touched/crossed below MA, today recovered above
            ma10_reclaim = (
                pd.notna(prev.ma10_dist) and prev.ma10_dist <= 0
                and pd.notna(curr.ma10_dist) and curr.ma10_dist > 0
            )
            ma20_reclaim = (
                pd.notna(prev.ma20_dist) and prev.ma20_dist <= 0
                and pd.notna(curr.ma20_dist) and curr.ma20_dist > 0
            )

            if not (ma10_reclaim or ma20_reclaim):
                continue

            # Volume should be contracting on the pullback (below average)
            rvol = curr.rvol20 if pd.notna(curr.rvol20) else 2.0
            if rvol > 1.0:
                continue

            disregarded_count = _try_add_event(
                events_map, df_day, idx, 'ema_reclaim', disregarded_count)

    # --- 8. Identify Selloff Events (PM-08 Overnight Reversal candidates) ---
    # Condition: pct < -2%, rvol20 > 1.5, Stage 2 uptrend (ma200_dist > 0)
    if all(c in df_day.columns for c in ['pct', 'rvol20', 'ma200_dist']):
        selloff_condition = (
            (df_day['pct'] < -2)
            & (df_day['rvol20'] > 1.5)
            & (df_day['ma200_dist'] > 0)
        ...
        for reaction_date in df_day.index[selloff_condition]:
            reaction_idx = df_day.index.get_loc(reaction_date)
            disregarded_count = _try_add_event(
                events_map, df_day, reaction_idx, 'selloff', disregarded_count)
    ...
    for reaction_date in sorted(events_map.keys()):
        ...
        # Boundary check for SPY too
        if idx_spy < OFFSET_DAYS or idx_spy >= len(df_spy_day) - OFFSET_DAYS:
            continue
        ...
        event_row = {
            ...
            # Flat event type flags for fast filtering
            ...
            'evt_episodic_pivot': ('episodic_pivot' in event_types),
            'evt_pre_earnings': ('pre_earnings' in event_types),
            'evt_ema_reclaim': ('ema_reclaim' in event_types),
            'evt_selloff': ('selloff' in event_types),

            'is_etf': is_etf,
            'eps': meta.get('eps', np.nan),
            'eps_est': meta.get('eps_est', np.nan),
            'earnings_when': meta.get('earnings_when', np.nan),
            'gappct': df_day.iloc[reaction_idx].gappct if 'gappct' in df_day.columns else np.nan,
            'market_cap': np.nan,
            ...
            # PEAD columns
            'sue': meta.get('sue', np.nan),
            'surprise_dir': meta.get('surprise_dir', 'unknown') if is_earnings else np.nan,
            'close_in_range': meta.get('close_in_range', np.nan),
            'consecutive_beats': meta.get('consecutive_beats', 0) if is_earnings else np.nan,
...
all_data = []
all_data_filename = f'{base_path}/all.parquet'
...
#%% Split file into chunks by year starting from < 2010, 2011, ... 2026
previous_year = 1900
for year in range(2010, 2027):
    filename = f'{base_path}/all_{year}.parquet'
