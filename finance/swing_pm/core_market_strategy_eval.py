...
def analyze_indicator_influence_ml(df_eval, name):
  ...
  features = ['gappct', 'hurst', 'ac_lag_1', 'ac_lag_5', 'ac_lag_21','ema20_slope', 'ema50_slope', 'ema100_slope', 'ema200_slope',
...
def _calculate_margin_for_intervals(df_filtered, freq_weeks=[1, 2, 3, 4]):
  ...
  for wk in freq_weeks:
    ...
    interval_margins[f'{wk}wk'] = df_m.set_index('time')['total_margin'].resample('D').max().fillna(method='ffill')
...
def _plot_permutation_results(df_res, best_res, df_baseline, name, num_features, logic_str):
    ...
    ax_margin.set_title("Capital Required by Interval"); ax_margin.set_ylabel("Margin ($)")
    ax_margin.legend(loc='upper left', fontsize=8); ax_margin.grid(True, alpha=0.2)
    ...
    plt.suptitle(f"STRATEGY: {name} | COMPLEXITY: {num_features} FEATURES", fontsize=10, fontweight='bold')
    plt.tight_layout()
